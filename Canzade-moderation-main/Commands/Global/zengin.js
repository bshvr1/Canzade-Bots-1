...
class Booster extends Command {
  ...
  if(!message.member.roles.cache.has(Config.Role.Booster)) return message.react(Config.Others.RedTick)
  const nick = args.slice(1).filter(arg => isNaN(arg)).map(arg => arg[0].toUpperCase() + arg.slice(1).toLowerCase()).join(" ");
  if(!nick) return message.react(Config.Others.Red_Tick)
  ...
  if (banNum == Config.Limit.Booster_Limit) return this.client.yolla("Gün içerisinde çok fazla jail işlemi uyguladığınız için komut geçici olarak kullanımınıza kapatılmıştır.", message.author, message.channel)
...
if(!message.member.user.username.includes(Config.Guild.Tag)) {
    message.member.setNickname(`${Config.Guild.Secondary_Tag} ${nick.charAt(0).toUpperCase() + nick.slice(1).toLowerCase()} ${yaş ? `| ${yaş}` : ``}`)
} else if (message.member.user.username.includes(Config.Guild.Tag)) {
message.member.setNickname(`${Config.Guild.Tag} ${nick.charAt(0).toUpperCase() + nick.slice(1).toLowerCase()} ${yaş ? `| ${yaş}` : ``}`)
} 
 return message.react(Config.Others.CheckTick)
