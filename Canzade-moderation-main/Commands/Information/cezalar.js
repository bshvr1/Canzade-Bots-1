...
class Cezalar extends Command {
    ...
    async run(message, args, perm) {
        ...
        await data.find({ user: user.id }).sort({ ihlal: "descending" }).exec(async (err, res) => {
            ...
            if (datax.length == 1) return this.client.yolla(`${user} kullanıcısının ceza bilgisi bulunmuyor.`, message.author, message.channel)
            ...
            /*message.channel.send(`<@${user.id}> kullanıcısının toplam ${datax.length-1} cezası bulunmakta son 15 ceza aşağıda belirtilmiştir.Tüm ceza bilgi dosyasını indirmek için 🚫 emojisine, ceza sayılarına bakmak için ❔ emojisine basabilirsin.Tekli bir cezaya bakmak için \`!ihlal ID\` komutunu uygulayınız. \`\`\`${outi}\`\`\` `).then(msg => {
                ...
                msg.awaitReactions((reaction, user) => user.id == message.author.id && (reaction.emoji.name == '🚫' || reaction.emoji.name == '❔'),
                    { max: 1, time: 30000 }).then(async collected => {
                        if (collected.first().emoji.toString() == '🚫') {
...
*/
message.channel.send(`<@${user.id}> kullanıcısının toplam ${datax.length-1} cezası bulunmakta son 15 ceza aşağıda belirtilmiştir.Tüm ceza bilgi dosyasını indirmek için 🚫 emojisine, ceza sayılarına bakmak için ❔ emojisine basabilirsin.Tekli bir cezaya bakmak için \`!ihlal ID\` komutunu uygulayınız. \`\`\`fix
${outi}
\`\`\` `).then(async msg => {
