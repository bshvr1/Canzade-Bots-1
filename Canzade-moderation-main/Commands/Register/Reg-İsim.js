...
`)
  ...
  const newnick = `${member.user.username.includes(Config.Guild.Tag) ? Config.Guild.Tag : (Config.Guild.Secondary_Tag ? Config.Guild.Secondary_Tag : (Config.Guild.Secondary_Tag || ""))} ${nick} | ${age}`;
...
${this.client.no} Kişinin Toplamda ${registerModel?.userNames?.length ?? 0} isim kayıtı bulundu.
${registerModel?.userNames?.map(x => `\`• ${x.nick}\` (${x.type.replace(`Erkek`, `<@&${Config.Role.Man[0]}>`).replace(`Kız`, `<@&${Config.Role.Woman[0]}>`)})`).slice(0, 10).join("\n ")}\n
Kişinin önceki isimlerine \`!isimler @üye\` komutuyla bakarak kayıt işlemini gerçekleştirmeniz önerilir.` 
: `
...
/*
   if(!member.user.username.includes(Config.Guild.Tag) && !member.roles.cache.has(Config.Booster_Role) && !member.roles.cache.has(GuildRole.Vip)) {
...
/*
  ...
  if (onay.size < 1) {
    ...
    this.client.channels.cache.get(Config.Log.Nick).send(new MessageEmbed() .setFooter(`Üyenin ceza puanı `+puan+``+ ` - ` + moment(Date.now()).format("LLL")+"")
  ...
  if (onayContent.includes(`k`)) {
   ...
   /*await member.roles.add(Role.Register.Woman)
    ...
    let roles = member.roles.cache.clone().filter(e => e.managed).map(e => e.id).concat(Config.Role.Woman);
    if(member.user.username.includes(Config.Guild.Tag)) roles.push(Config.Role.Family_Role)
    member.roles.set(roles).catch();
    ...
    const embed = new MessageEmbed()
      .setAuthor(message.author.tag, message.author.avatarURL({ dynamic: true }))
      .setColor("RANDOM")
      .setDescription(`${member.toString()} üyesine ${Config.Role.Woman.map(x => `<@&${x}>`)} rolleri verildi.`)
      .setFooter(`Üyenin ceza puanı `+puan+``);
    message.channel.send(embed)//.then(x => x.delete({timeout: 5000}))
    this.client.channels.cache.get(Config.Log.General_Chat).send(`Aramıza yeni biri katıldı! ${member.toString()} ona hoş geldin diyelim!`)
