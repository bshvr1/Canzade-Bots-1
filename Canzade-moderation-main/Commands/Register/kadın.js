...
class Kadın extends Command {
  constructor(client) {
      super(client, {
          ...
          usage: "kadın",
          aliases: ["k","kadın","woman","bayan","girl","kız"]
      });
  }
   ...
   if (message.guild.members.cache.has(member.id) && message.member.roles.highest.position <= message.guild.members.cache.get(member.id).roles.highest.position) return this.client.yolla("Kendi rolünden yüksek kişilere işlem uygulayamazsın!", message.author, message.channel)
    ...
    if(!registerData) {
      let newRegisterData = new Register({
        ...
        womanRegister: 0,
        manRegister: 0,
        userNames: [{ nick: `${member.user.username.includes(Config.Guild.Tag) ? Config.Guild.Tag : (Config.Guild.Secondary_Tag ? Config.Guild.Secondary_Tag : (Config.Guild.Secondary_Tag || ""))} ${nick} | ${age}`, type: `<@&${Config.Role.Woman[0]}>`}]
      }).save();
    } else {
      registerData.userNames.push({ nick: `${member.user.username.includes(Config.Guild.Tag) ? Config.Guild.Tag : (Config.Guild.Secondary_Tag ? Config.Guild.Secondary_Tag : (Config.Guild.Secondary_Tag || ""))} ${nick} | ${age}`, type: `<@&${Config.Role.Woman[0]}>`})
      registerData.save();
    }
