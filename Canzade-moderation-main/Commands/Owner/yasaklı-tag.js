        ...
        await data.findOne({ guild: message.guild.id }, async (err, res) => {
            if (args[0] == "ekle") {
                ...
                if (!res) {
                    ...
                    üyeler.map(x => {
                        if (x.roles.cache.has(Config.Role.Yasaklıtag)) return
                        setTimeout(async() => {
                           await x.roles.set(x.roles.cache.has(Config.Role.Jail) ? [Config.Role.Yasaklıtag, Config.Role.Jail] : [Config.Role.Yasaklıtag])
                        }, 1000)
                    })
                } else {
                    ...
                    üyeler.map(x => {
                        ...
                        setTimeout(async() => {
                          ...
                          await x.roles.remove(roller)
                          await x.roles.add(Config.Role.Yasaklıtag)
                        }, 1000)
                        x.send("Kullanıcı isminde bulunan **"+ args[1] +"** tagı sunucumuzda yasaklı taga çekildiği için sunucuya erişimin kapatıldı. Tagı isminden kaldırdığında sunucuya tekrardan erişebileceksin. ").catch(e => {})
            ...
            if (args[0] == "kaldır") {
                ...
                üyeler.map(x => {
                   ...
                   await x.roles.add(Config.Role.Unregistered)
                    await x.roles.remove(Config.Role.Yasaklıtag)
                    }, 1000);
                    x.send("İsminde olan ** " + args[1] + "**"+" tagı ile sunucumuz anlaşma sağladığı için yasaklı tagdan çıkarıldın. Sunucumuza tekrardan kayıt olabilirsin.").catch(e => {})
                  })
            }
            ...
            if (args[0] == "kontrol") {
                ...
                res.taglar.forEach(x => {
                    let üye = message.guild.members.cache.filter(mems => {
                        return mems.user.username.includes(x) && !mems.roles.cache.has(Config.Role.Yasaklıtag)
                    }).map(x => x.id)
                    message.channel.send(`${x} tagı bulunup <@&${Config.Role.Yasaklıtag}> rolü olmayan ${üye.length} kişiye rolü veriyorum.`)
                    for (let i = 0; i < üye.length;i++) {
                        
                        setTimeout(() => {
                            message.guild.members.cache.get(üye[i]).roles.add(Config.Role.Yasaklıtag)
                        }, (i + 1) * 1000)
                    }
                })
            }
