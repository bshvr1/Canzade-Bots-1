...
class Unregister extends Command {
    ...
    async run(message, args, client) {
        if (!message.member.hasPermission("ADMINISTRATOR") && !Config.Permissions.Register.some(role => message.member.roles.cache.has(role))) return this.client.yolla("Bunu yapmak için yeterli bir yetkiye sahip değilsin.", message.author, message.channel);
        ...
        let roles = victim.roles.cache.clone().filter(e => e.managed).map(e => e.id).concat(Config.Role.Unregistered);
