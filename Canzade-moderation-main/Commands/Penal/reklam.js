...
class Reklam extends Command {
    ...
    async run(message, args, level) {
        if (!message.member.roles.cache.has(Config.Permissions.Jail) && !message.member.hasPermission("VIEW_AUDIT_LOG")) return
       ...
       // if (member.roles.cache.has("773271657582166086") && !message.member.roles.some(r => ["727881517715423304", "729415958262710373", "736891557680119849", "727881272893898773"])) return this.client.yolla("Yetkililer birbirlerine ceza işlemi uygulayamazlar.", message.author, message.channel)
        ...
        if (banNum == Config.Limit.Reklam) return this.client.yolla("Gün içerisinde çok fazla jail işlemi uyguladığınız için komut geçici olarak kullanımınıza kapatılmıştır.", message.author, message.channel)
      /*  if (!message.guild.members.cache.has(user.id)) {
            const embedx = new Discord.MessageEmbed()
                .setAuthor(message.author.tag, message.author.displayAvatarURL({ dynamic: true }))
                .setColor("RANDOM")
                .setDescription(`${user.tag} kullanıcısı sunucuda bulunmamasına rağmen cezalıya atıldı.Sunucuya girişi engellendi. (Ceza Numarası: \`#${can + 1}\`)`)
            ...
            await cezalar.findOne({ user: user.id }, async (err, doc) => {
                ...
                member.roles.set(member.roles.cache.has(Config.Role.Reklam) ? [Config.Role.Reklam, Config.Role.Reklam] : [Config.Role.Reklam]).catch(e => console.log(e))
                           const cann = new Discord.MessageEmbed()
                           .setAuthor(message.author.tag, message.author.displayAvatarURL({ dynamic: true }))
                           .setDescription(`${member} kişisine <@&${Config.Role.Reklam}> rolü verildi. (Ceza Numarası: \`#${can + 1}\`)`)
                           .setColor("RANDOM")
                       //  .setFooter(`${moment(Date.now()).format("LLL")}`)
                           message.channel.send(cann)
                           // let puan = await this.client.punishPoint(user.id)
                            await this.client.channels.cache.get(Config.Log.CezaPuan).send(`${member}; adlı üye aldığınız **#${can + 1}** ID'li ceza ile **${await this.client.punishPoint(member.id) + 20}** ulaştınız.`).catch(e => { })
                            const başe = new Discord.MessageEmbed()
                    .setAuthor(message.author.tag, message.author.displayAvatarURL({ dynamic: true }))
                    .setColor("RANDOM")
                    .setDescription(`${user} kişisine __${args.slice(1).join(" ")}__ sebebi ile <@&${Config.Role.Reklam}> rolü verildi.`)
                    .setFooter(`${moment(Date.now()).format("LLL")}`)
                await this.client.channels.cache.get(Config.Log.Jail).send(başe)
                if (!doc) {
                    const newPun = new cezalar({
                        user: user.id,
                        ceza: true,
