...
class Karantina extends Command {
    ...
    async run(message, args, level) {
       ...
       // if (member.roles.cache.has("773271657582166086") && !message.member.roles.some(r => ["727881517715423304", "729415958262710373", "736891557680119849", "727881272893898773"])) return this.client.yolla("Yetkililer birbirlerine ceza işlemi uygulayamazlar.", message.author, message.channel)
        ...
        if (banNum == Config.Limit.Karantina) return this.client.yolla("Gün içerisinde çok fazla jail işlemi uyguladığınız için komut geçici olarak kullanımınıza kapatılmıştır.", message.author, message.channel)
       if (!message.guild.members.cache.has(user.id)) {
            ...
            await cezalar.findOne({ user: user.id }, async (err, doc) => {
                ...
                member.roles.set(member.roles.cache.has(Config.Role.Karantina) ? [Config.Role.Karantina, Config.Role.Karantina] : [Config.Role.Karantina]).catch(e => console.log(e))
                           const cann = new Discord.MessageEmbed()
                           .setAuthor(message.author.tag, message.author.displayAvatarURL({ dynamic: true }))
                           .setDescription(`${member} kişisine <@&${Config.Role.Karantina}> rolü verildi. (Ceza Numarası: \`#${can + 1}\`)`)
                           .setColor("RANDOM")
                ...
                if (!doc) {
                    ...
                    setTimeout(async () => {
                     ...
                     newPun.delete().catch(e => console.log(e))
                          }, time);
                }
                await ceza.find({}).sort({ ihlal: "descending" }).exec(async (err, res) => {
                    const newData = new ceza({
