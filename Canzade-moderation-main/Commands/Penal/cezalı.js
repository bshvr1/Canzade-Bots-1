...
class Cezalı extends Command {
    ...
    async run(message, args, level) {
       ...
       // if (member.roles.cache.has("773271657582166086") && !message.member.roles.some(r => ["727881517715423304", "729415958262710373", "736891557680119849", "727881272893898773"])) return this.client.yolla("Yetkililer birbirlerine ceza işlemi uygulayamazlar.", message.author, message.channel)
        ...
        if (banNum == Config.Limit.Jail) return this.client.yolla("Gün içerisinde çok fazla jail işlemi uyguladığınız için komut geçici olarak kullanımınıza kapatılmıştır.", message.author, message.channel)
        if (!message.guild.members.cache.has(user.id)) {
            ...
            await cezalar.findOne({ user: user.id }, async (err, doc) => {
                ...
                member.roles.set(member.roles.cache.has(Config.Role.Jail) ? [Config.Role.Jail, Config.Role.Jail] : [Config.Role.Jail]).catch(e => console.log(e))
                           const cann = new Discord.MessageEmbed()
                           .setAuthor(message.author.tag, message.author.displayAvatarURL({ dynamic: true }))
                           .setDescription(`${member} kişisine <@&${Config.Role.Jail}> rolü verildi. (Ceza Numarası: \`#${can + 1}\`)`)
                           .setColor("RANDOM")
                       //  .setFooter(`${moment(Date.now()).format("LLL")}`)
                           message.channel.send(cann)
                           // let puan = await this.client.punishPoint(user.id)
                            await this.client.channels.cache.get(Config.Log.CezaPuan).send(`${member}; adlı üye aldığınız **#${can + 1}** ID'li ceza ile **${await this.client.punishPoint(member.id) + 15}** ulaştınız.`).catch(e => { })
                            const başe = new Discord.MessageEmbed()
                    .setAuthor(message.author.tag, message.author.displayAvatarURL({ dynamic: true }))
                    .setColor("RANDOM")
                    .setDescription(`${user} kişisine __${args.slice(1).join(" ")}__ sebebi ile <@&${Config.Role.Jail}> rolü verildi.`)
                    .setFooter(`${moment(Date.now()).format("LLL")}`)
