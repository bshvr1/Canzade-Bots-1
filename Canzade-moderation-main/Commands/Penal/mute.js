...
class Mute extends Command {
    ...
    async run(message, args, perm) {
        ...
        if (user.roles.cache.has(Config.Role.ChatMute)) return this.client.yolla("Kullanıcı zaten susturulmuş durumda.", message.author, message.channel)
        let time = ms(args[1]);
        let muteTime = time
        ...
        let can = count
        user.roles.add(Config.Role.ChatMute)
        await message.channel.send(`${this.client.ok} <@${user.id}> kişisi ${await this.client.turkishDate(time)} ${ms(args[1]) < muteTime ? `(\`kullanıcı daha önceden cezalı olduğu için cezasına +${await this.client.turkishDate(muteTime - time)} eklendi.\`) ` : `` } boyunca metin kanallarında susturuldu. (Ceza Numarası: \`#${can + 1}\`)`)
      //  let puan = await this.client.punishPoint(user.id)
        await this.client.channels.cache.get(Config.Log.CezaPuan).send(`${user}; adlı üye aldığınız **#${can + 1}** ID'li ceza ile **${await this.client.punishPoint(user.id) + 8}** ulaştınız.`).catch(e => { })
        const mutelendı = new Discord.MessageEmbed()
            ...
            await this.client.channels.cache.get(Config.Log.ChatMute).send(mutelendı)
