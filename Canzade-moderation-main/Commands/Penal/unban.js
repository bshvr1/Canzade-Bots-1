                  ...
                  if(!ban){
                    ...
                    await db.findOne({userid: res.id}, async(err,dbres) => {
                        if(!dbres) {
                            ...
                            embed.setDescription(`**${res.tag}** kullanıcısının yasağı kaldırıldı. (\`#${can + 1}\`)`)
                            message.channel.send(embed)
                            this.client.channels.cache.get(Config.Log.Unban).send(new Discord.MessageEmbed()
                            .setColor("RANDOM")
                            ...
                            await message.guild.members.unban(res.id)
                            embed.setDescription(`**${res.tag}** kullanıcısının yasağı kaldırıldı. (\`#${can + 1}\`) `)
                            this.client.channels.cache.get(this.client.Perm.Log.Unban).send(new Discord.MessageEmbed()
                            .setColor("RANDOM")
                            .setAuthor(message.author.tag, message.author.avatarURL({ dynamic: true }))
                            .setFooter(moment(Date.now()).format("LLL"))
                            .setDescription(`**${res.tag}** üyesinin yasağı **${message.author}** tarafından kaldırıldı.`)
